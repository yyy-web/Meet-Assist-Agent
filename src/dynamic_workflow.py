...
# LangChain core messages  (unchanged — langchain-core is stable)
from langchain_core.messages import HumanMessage, AIMessage, SystemMessage, BaseMessage, ToolMessage
from langchain_core.tools import tool

# PromptTemplate: moved to langchain_core in LangChain 1.x
from langchain_core.prompts import PromptTemplate            # ← was: from langchain.prompts

# Retriever tool: moved in langchain.tools (langchain package, not core)
from langchain_core.tools.retriever import create_retriever_tool

# LLM: langchain-openai package (unchanged)
from langchain_openai import ChatOpenAI

# Chroma: now lives in langchain_chroma package
from langchain_chroma import Chroma                          # ← was: from langchain_chroma (same, but ensure package >=0.2)

# PDF loader: langchain-community (unchanged path)
from langchain_community.document_loaders import PyPDFLoader

# *** TEXT SPLITTER: moved to langchain_text_splitters package ***
from langchain_text_splitters import RecursiveCharacterTextSplitter   # ← was: from langchain.text_splitter

# Embeddings: still in langchain-community (unchanged)
...
def build_workflow():
    ...
    workflow.add_node("tools", lambda state: create_dynamic_tool_node(state))
