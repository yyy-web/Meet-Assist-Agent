...
st.markdown("*AI-powered technical interviews with voice interaction*")
...
if input_method == "Voice":
    ...
    language = st.sidebar.selectbox(
        ...
        help="Force recognition in English or auto-detect language"
    )
    ...
    **Language:**
    - en: Force English recognition (recommended for English speakers)
...
if input_method == "Voice":
    ...
    if audio_bytes:
        ...
        if duration > MAX_RECORD_SECONDS:
            ...
            with st.status(f"Transcribing audio with Whisper ({model_size} model, language={language})...", expanded=True) as status:
                transcribed_text, stt_error = whisper_transcribe(audio_path, model_size=model_size, language=language)
                if stt_error:
